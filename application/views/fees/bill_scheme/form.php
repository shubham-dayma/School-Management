			 ...
			 <?php } ?>
				  <form class="form-horizontal" method="post">
					  <div class="box-body">
					  	<div class="col-md-6">
							...
							<div class="form-group">
							  ...
							  <div class="col-sm-8">
								<textarea class="form-control" id="desc" name="desc"><?php echo (!empty ($row->desc)) ? $row->desc : set_value('desc');  ?></textarea>
							  </div>
							</div>
						</div>
						<div class="col-md-6">
							<?php if (!empty ($classes)) { foreach ($classes as $class) { 
								  $assign_class = $this->Bills->get_scheme_class(@$row->id,$class->id);	
								  $check = '';
								  if ($class->id == @$assign_class->class_id){
								  	$check = 'checked="checked"';
								  }
							?>
								<div class="form-group">
									<input type="checkbox" name="classes[]" <?php echo $check ?> value="<?php echo $class->id ?>" />
					...
					<div class="box-footer">
						<a href="<?php echo site_url ('fees/bill_scheme'); ?>" class="btn btn-default btn-flat"><?php echo lang('cancel') ?></a>
