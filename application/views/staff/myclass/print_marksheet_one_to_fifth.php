...
<body>
	...
	<div id="marksheet">
		<div class="school_logo_name">
			...
			<p class="school_name"> <?php echo $school_name; ?></p>
		</div>
				...
				<table cellpadding="5" cellspacing="1" border="1px" class="marksheet-data">
					...
					<tbody>
							<?php if (!empty ($subjects)) { foreach ($subjects as $subject){									
								if ($subject->add_in_marksheet == 1) {
							?>
							<tr class="main-subject">
								<td><?php echo $subject->name // $subject->subject_code ?></td>
							 ...
							 <?php }else { $co_scolastic_subs[] = $subject; } ?>
							<?php } } ?>
							<?php //echo '<pre>';print_r($student->id);die; ?>
							<tr class="text-align-center bold">
								<td ><?php echo lang('marksheet_non-co_scolastic_total_marks') ?></td>
								<td><?php echo $anual_marksheet->per_max_marks ;?></td>
								<td><?php echo $anual_marksheet->per_min_marks ;?></td>
								<td><?php echo $anual_marksheet->pre_obtain_marks ?></td>
							</tr>
							<?php if (!empty ($co_scolastic_subs)) { ?>
							   <tr>
							  	<td colspan="4" ><?php echo lang('marksheet_co_scolastic_subjects') ?></td>
							  		

							  </tr>
							   <?php foreach ($co_scolastic_subs as $co_scolastic_sub) { ?>
							  <tr class="grade-subject">
								<td><?php echo /*$co_scolastic_sub->subject_code.' '. */$co_scolastic_sub->name ?></td>
								<td><?php echo $co_scolastic_sub->max_marks ?></td>
								<td><?php echo $co_scolastic_sub->min_marks ?></td>
								<?php $entered_marks = $this->Marks_zone->get_marks_entery_anual($co_scolastic_sub->id,$section_detail->id,$student->id); ?>
								<td><?php echo @$entered_marks ->optain_marks; ?>
									<?php if (!empty($grades)) { foreach ($grades as $grade) { 
											if ( $grade->marks_from <= @$entered_marks ->optain_marks && @$entered_marks ->optain_marks <= $grade->marks_to){
												echo $grade->name;
							...
							<?php }$co_scolastic_subs = '';} ?>
							<tr class="grand-total">
								<td>Total Marks</td>
								<td><?php echo ($anual_marksheet->max_marks - $anual_marksheet->per_max_marks) ;?></td>
								<td><?php echo ($anual_marksheet->min_marks - $anual_marksheet->per_min_marks) ;?></td>
								<td><?php echo ($anual_marksheet->obtain_marks - $anual_marksheet->pre_obtain_marks) ?></td>
