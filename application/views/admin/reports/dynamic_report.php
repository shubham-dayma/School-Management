              ...
              <form method="post">
				...
				<div class="row">
					...
					<div class="dynamic col-md-<?php echo $cols ?>">
						<select name="section_id">
							...
							<?php if (!empty($classes)) { foreach ($classes as $class) { $sections = $this->custom_lib->get_where('sections' , 'class_id',$class->id)->result();
							 ...
							 ?>
								<option value="<?php echo $section->id?>" <?php echo $sel; ?>> <?php echo ucwords($class->name).' '.ucwords($section->name) ?></option>
								<?php }} ?>
							<?php }} ?>
						</select>
					</div>
				...
				<div class="row">
					<div class="col-md-12" style="padding: 20px;text-align: right;">
						<input type="submit" name="submit" class="btn bg-purple btn-flat" value="<?php echo lang('submit') ?>" />
					</div>
				</div>	
			  </form>
            </div>
